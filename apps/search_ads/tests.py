...
class SearchAdsModerationTests(TestCase):
    """Горячий путь выдачи не отдаёт непромодерированную рекламу"""
    ...
    def setUp(self):
        ...
        # Создаём заказ через API — как это делает фронтенд
        response = self.client_adv.post('/api/search_ads/order/create/', {
            'channel_id': 'ch_1',
            'channel_name': 'Новостной канал',
            'order_name': 'Заказ',
            'tags': ['новости'],
            'spm': '5.00',
            'budget': '50.00',
    ...
    def _search(self, viewer_id='viewer_1'):
        return self.server.post('/api/search_ads/order/search/',
                                {'tag': 'новости', 'viewer_id': viewer_id},
    ...
    def test_user_cannot_activate_pending_order(self):
        """Юзер не может включить заказ, который ещё не прошёл модерацию"""
        response = self.client_adv.post('/api/search_ads/order/change_status/', {
    ...
    def test_user_cannot_activate_blocked_order(self):
        self.order.approve(self.moderator)
        self.order.refresh_from_db()
        self.order.block(self.moderator, 'Нарушение')
        ...
        self.order.refresh_from_db()
        self.assertFalse(self.order.is_active)
        self.assertEqual(self.order.status, ModerationStatus.BLOCKED)

    def test_user_sees_status_and_reason(self):
        """Пользователь видит статус заказа и причину отклонения"""
        self.order.reject(self.moderator, 'Запрещённая тематика')

        response = self.client_adv.get(f'/api/search_ads/order/{self.order.order_id}/detail/')
        self.assertEqual(response.status_code, 200)
